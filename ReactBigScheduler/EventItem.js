...
class EventItem extends Component {
    ...
    initStartDrag = (ev) => {
        ...
        if(supportTouch) {
            ...
            if (typeof(ev.buttons) !== 'undefined' && ev.buttons !== 1) return;
            clientX = ev.clientX;
        }
        this.setState({
            startX: clientX
    ...
    stopStartDrag = (ev) => {
        ...
        if (hasConflict) {
            ...
            if (typeof(updateEventStart) != 'undefined')
    ...
    stopEndDrag = (ev) => {
        ...
        if (hasConflict) {
            ...
            if (typeof(updateEventEnd) != 'undefined')
    ...
    render() {
        ...
        const a = <a className="timeline-event" style={{left: left, width: width, top: top}} onClick={() => { if(!!eventItemClick) eventItemClick(schedulerData, eventItem);}}>
            {eventItemTemplate}
            {startResizeDiv}
            {endResizeDiv}
        </a>;
        ...
        return (
            isDragging ? null : ( schedulerData._isResizing() || config.eventItemPopoverEnabled==false || eventItem.showPopover==false ?
                    ...
                    <PopoverComponent content={content} placement={'bottom-start'} trigger={'hover'}>
                        {
                            connectDragPreview(
                                connectDragSource(a)
                            )
    ...
    startResizable = (props) => {
        ...
        return config.startResizable === true && isInPopover === false
            && (typeof(eventItem.resizable) == 'undefined' || eventItem.resizable !== false)
            && (typeof(eventItem.startResizable) == 'undefined' || eventItem.startResizable !== false);
    }
    ...
    endResizable = (props) => {
        ...
        return config.endResizable === true && isInPopover === false
            && (typeof(eventItem.resizable) == 'undefined' || eventItem.resizable !== false)
            && (typeof(eventItem.endResizable) == 'undefined' || eventItem.endResizable !== false);
    }

    subscribeResizeEvent = (props) => {
        if (typeof(this.startResizer) !== 'undefined'&&this.startResizer!==null) {
            if(supportTouch) {
        ...
        if (typeof(this.endResizer) !== 'undefined'&&this.endResizer!==null) {
            if(supportTouch) {
