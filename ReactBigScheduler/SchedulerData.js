...
export default class SchedulerData {
    constructor(date = moment().format(DATE_FORMAT), viewType = ViewTypes.Week,
        ...
        this.config = typeof(newConfig) == 'undefined' ? config : { ...config, ...newConfig };
        this._validateMinuteStep(this.config.minuteStep);
        this.behaviors = typeof(newBehaviors) == 'undefined' ? behaviors : { ...behaviors, ...newBehaviors };
    ...
    setResources(resources, expandedStatusMap) {
        this._validateResource(resources);
        this.resources = Array.from(new Set(resources));
        this._createRenderData(expandedStatusMap);
    ...
    setEvents(events, expandedStatusMap) {
    ...
    setViewType(viewType = ViewTypes.Month, showAgenda = false, isEventPerspective = false, expandedStatusMap) {
        this.showAgenda = showAgenda;
        this.isEventPerspective = isEventPerspective;
        ...
        if(this.viewType !== viewType) {
            ...
            if(viewType === ViewTypes.Custom || viewType === ViewTypes.Custom1 || viewType === ViewTypes.Custom2) {
                ...
                if(this.viewType < viewType){
                    ...
                    if(typeof(this.selectDate) !== 'undefined') {
    ...
    //     this.renderData = this.renderData.map((item) => {
    ...
    updateEventStart(event, newStart, expandedStatusMap) {
    ...
    updateEventEnd(event, newEnd, expandedStatusMap) {
        event.end = newEnd;
        this._createRenderData(expandedStatusMap);
    }

    moveEvent(event, newSlotId, newSlotName, newStart, newEnd, expandedStatusMap){
        this._detachEvent(event);
    ...
    _resolveDate(num, date = undefined){
        ...
        if(this.viewType === ViewTypes.Week) {
            this.startDate = typeof(date) != 'undefined' ? this.localeMoment(date).startOf('week').format(DATE_FORMAT)
                : this.localeMoment(this.startDate).add(num, 'weeks').format(DATE_FORMAT);
            this.endDate = this.localeMoment(this.startDate).endOf('week').format(DATE_FORMAT);
        }
        else if(this.viewType === ViewTypes.Day) {
            this.startDate = typeof(date) != 'undefined' ? this.selectDate
                : this.localeMoment(this.startDate).add(num, 'days').format(DATE_FORMAT);
            this.endDate = this.startDate;
        }
        else if(this.viewType === ViewTypes.Month) {
            this.startDate = typeof(date) != 'undefined' ? this.localeMoment(date).startOf('month').format(DATE_FORMAT)
                : this.localeMoment(this.startDate).add(num, 'months').format(DATE_FORMAT);
            this.endDate = this.localeMoment(this.startDate).endOf('month').format(DATE_FORMAT);
        }
        else if(this.viewType === ViewTypes.Quarter) {
            this.startDate = typeof(date) != 'undefined' ? this.localeMoment(date).startOf('quarter').format(DATE_FORMAT)
                : this.localeMoment(this.startDate).add(num, 'quarters').format(DATE_FORMAT);
            this.endDate = this.localeMoment(this.startDate).endOf('quarter').format(DATE_FORMAT);
        }
        else if(this.viewType === ViewTypes.Year) {
            this.startDate = typeof(date) != 'undefined' ? this.localeMoment(date).startOf('year').format(DATE_FORMAT)
            ...
            if(typeof(this.behaviors.getCustomDateFunc) != 'undefined'){
    ...
    _createInitRenderData(isEventPerspective, eventGroups, resources, headers, expandedStatusMap) {
        ...
        slots.forEach((slot) => {
            ...
            let slotRenderData = {
                ...
                indent: 0,
                hasChildren: false,
                expanded: ((typeof (expandedStatusMap) === 'undefined') || (typeof (expandedStatusMap[slot.id]) === 'undefined')) ? true : expandedStatusMap[slot.id].expanded,
                render: ((typeof (expandedStatusMap) === 'undefined') || (typeof (expandedStatusMap[slot.id]) === 'undefined')) ? true : expandedStatusMap[slot.id].render,
            };
            let id = slot.id;
            let value = undefined;
            if(slotMap.has(id)) {
        ...
        while(slotStack.length > 0) {
            ...
            if(currentNode.data.indent > 0) {
                currentNode.data.render = (typeof (expandedStatusMap) === 'undefined' || typeof (expandedStatusMap[currentNode.data.slotId]) === 'undefined' || typeof (expandedStatusMap[currentNode.data.slotId].render) === 'undefined') ? this.config.defaultExpanded : expandedStatusMap[currentNode.data.slotId].render;
            }
            if(currentNode.children.length > 0) {
                currentNode.data.hasChildren = true;
                currentNode.data.expanded = (typeof (expandedStatusMap) === 'undefined' || typeof (expandedStatusMap[currentNode.data.slotId]) === 'undefined' || typeof (expandedStatusMap[currentNode.data.slotId].expanded) === 'undefined') ? this.config.defaultExpanded : expandedStatusMap[currentNode.data.slotId].expanded;
            }
    ...
    _createRenderData(expandedStatusMap) {
        let initRenderData = this._createInitRenderData(this.isEventPerspective, this.eventGroups, this.resources, this.headers, expandedStatusMap);
        ...
        if (cellMaxEventsCount <= cellMaxEventsCountValue || typeof(this.behaviors.getSummaryFunc) !== 'undefined') {
            initRenderData.forEach((resourceEvents) => {
                ...
                resourceEvents.headerItems.forEach((headerItem) => {
                    ...
                    if (typeof(this.behaviors.getSummaryFunc) !== 'undefined') {
                        let events = [];
